...
This command allows users of the AddressBook application to add optional remarks to people in their status book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/status/logic/command` directory.
...
```java
package seedu.status.logic.commands;

import seedu.status.model.Model;

/**
 * Changes the remark of an existing internApplication in the status book.
 */
...
```java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the internApplication identified "
            + "by the index number used in the last internApplication listing. "
...
```java
import static seedu.status.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
    ...
    /**
     * @param index of the internApplication in the filtered internApplication list to edit the remark
     * @param remark of the internApplication to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.status.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of internApplication data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the internApplication’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a internApplication.

### Add a new `Remark` class

Create a new `Remark` in `seedu.status.model.application`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each internApplication.

Simply add the following to [`seedu.status.ui.PersonCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
You must delete AddressBook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause AddressBook to default to an empty status book!
...
```java
public PersonCard(Person internApplication, int displayedIndex) {
    //...
    remark.setText(internApplication.getRemark().value);
}
```
...
//...
     ...
     * {@code internApplicationToEdit}.
     */
    private String generateSuccessMessage(Person internApplicationToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, internApplicationToEdit);
    }
```
