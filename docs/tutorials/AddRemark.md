...
/**
 * Changes the remark of an existing internApplication in the address book.
 */
...
Create a new `Remark` in `seedu.address.model.internApplication`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
