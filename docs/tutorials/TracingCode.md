...
According to the sequence diagram you saw earlier (and repeated above for reference), the `UI` component yields control to the `Logic` component through a method named `execute`. Searching through the code base for an `execute()` method that belongs to the `Logic` component yields a promising candidate in `seedu.status.logic.Logic`.
...
A quick look at the `seedu.status.logic.Logic` (an extract given below) confirms that this indeed might be what we’re looking for.
...
1. Now, step over until you read the statement that calls the `execute()` method of the `EditCommand` object received, and step into that `execute()` method (partial code given below):
   ...
   **`EditCommand#execute()`:**
   ...
   public CommandResult execute(Model model) throws CommandException {
       ...
       Person editedInternApplication = createEditedPerson(internApplicationToEdit, editPersonDescriptor);
       if (!internApplicationToEdit.isSamePerson(editedInternApplication) && model.hasPerson(editedInternApplication)) {
           throw new CommandException(MESSAGE_DUPLICATE_PERSON);
       }
       model.setPerson(internApplicationToEdit, editedInternApplication);
       model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);
       return new CommandResult(String.format(MESSAGE_EDIT_PERSON_SUCCESS, editedInternApplication));
   }
   ```

1. As suspected, `command#execute()` does indeed make changes to the `model` object. Specifically,
   * it uses the `setPerson()` method (defined in the interface `Model` and implemented in `ModelManager` as per the usual pattern) to update the internApplication data.
   * it uses the `updateFilteredPersonList` method to ask the `Model` to populate the 'filtered list' with _all_ internApplications.<br>
     FYI, The 'filtered list' is the list of internApplications resulting from the most recent operation that will be shown to the user immediately after. For the `edit` command, we populate it with all the internApplications so that the user can see the edited internApplication along with all other internApplications. If this was a `find` command, we would be setting that list to contain the search results instead.<br>
     To provide some context, given below is the class diagram of the `Model` component. See if you can figure out where the 'filtered list' of internApplications is being tracked.
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save the status book in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Person`

    6.  Add a new entity to the status book
