...
This tutorial aims to give you some practice on such a code 'removal' activity by removing the `status` field from `Person` class.

<box type="success">

**If you have done the [Add `remark` command tutorial](AddRemark.html)  already**, you should know where the code had to be updated to add the field `remark`. From that experience, you can deduce where the code needs to be changed to _remove_ that field too. The removing of the `status` field can be done similarly.
<br>
<br>
...
The `status` field in `Person` is actually an instance of the `seedu.letsgethired.model.application.Status class. Since removing the `Address` class will break the application, we start by identifying `Address`'s usages. This allows us to see code that depends on `Address` to function properly and edit them on a case-by-case basis. Right-click the `Address` class and select `Refactor` \> `Safe Delete` through the menu.
...
1. Safe delete the field `status` in `EditPersonDescriptor`.
...
1. Remove the usages of `status` and select `Do refactor` when you are done.

   <box type="tip" seamless>

   **Tip:** Removing usages may result in errors. Exercise discretion and fix them. For example, removing the `status` field from the `Person` class will require you to modify its constructor.
   </box>
...
Unfortunately, there are usages of `Address` that IntelliJ IDEA cannot identify. You can find them by searching for instances of the word `status` in your code (`Edit` \> `Find` \> `Find in path`).

Places of interest to look out for would be resources used by the application. `main/resources` contains images and `fxml` files used by the application and `test/resources` contains test data. For example, there is a `$status` in each `PersonCard` that has not been removed nor identified.

![$status](../images/remove/$status.png)
...
In `src/test/data/`, data meant for testing purposes are stored. While keeping the `status` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
You can go through each individual `json` file and manually remove the `status` field.
